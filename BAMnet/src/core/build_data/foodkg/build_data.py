    ...
    for qid, each in enumerate(qa):
        ...
        if each['qType'] != 'comparison':
            ...
            if augment_similar_dishs:
                subgraph = augment_kb_subgraph_with_similar_dishs(kb, topic_key_list[0], each['similar_recipes'], additional_dish_info)
            else:
            ...
            for tag_index in range(1, len(topic_key_list)):
                if augment_similar_dishs:
                    subgraph = augment_kb_subgraph_with_similar_dishs(kb, topic_key_list[tag_index], each['similar_recipes'], additional_dish_info)
                else:
