...
class KBQA(object):
    ...
    def simple_answer(self, question, topic_entity, entities):
        question_dict = {'qType': 'simple',
                         ...
                         'rel_path': [],
                         'answers': []}
        data_vec = build_all_data([question_dict], self.local_kb, self.entity2id, self.entityType2id, self.relation2id,
                                  self.vocab2id)
        queries, raw_queries, query_mentions, query_marks, memories, cand_labels, _, _, cand_rel_paths, cand_ids = data_vec
        queries, query_words, query_marks, query_lengths, memories_vec, _ = vectorize_data(queries, query_mentions,
                                                                                           query_marks, memories, \
                                                                                           max_query_size=self.config[
                                                                                               'query_size'], \
                                                                                           max_ans_type_bow_size=
                                                                                           self.config[
                                                                                               'ans_type_bow_size'], \
                                                                                           max_ans_path_bow_size=
                                                                                           self.config[
                                                                                               'ans_path_bow_size'], \
                                                                                           max_ans_path_size=
                                                                                           self.config['ans_path_size'], \
                                                                                           vocab2id=self.vocab2id, \
                                                                                           fixed_size=True, \
                                                                                           verbose=False)

        pred, query_attn = self.predict(
            [memories_vec, queries, query_words, raw_queries, query_mentions, query_marks, query_lengths], cand_labels)
        if len(pred[0]) == 0:
            return [], [], []
    ...
    def comparision_answer(self, question, topic_entity_1, topic_entity_2, entities):
        ...
        intermediate_pred, query_attn = self.predict(
            [memories_vec, queries, query_words, raw_queries, query_mentions, query_marks, query_lengths], cand_labels)

        if topic_entity_1 in self.local_kb:
            topic_entity_1_id = self.local_kb[topic_entity_1]['uri']
            topic_entity_1_name = self.local_kb[topic_entity_1]['name'][0] if len(
                self.local_kb[topic_entity_1]['name']) > 0 else 'No answer'
        ...
        if topic_entity_2 in self.local_kb:
            ...
            topic_entity_2_name = self.local_kb[topic_entity_2]['name'][0] if len(
                self.local_kb[topic_entity_2]['name']) > 0 else 'No answer'
    ...
    def personalized_answer(self, question, topic_entity, entities, multi_tag_type='none',
        ...
        queries, raw_queries, query_mentions, query_marks, memories, cand_labels, _, _, cand_rel_paths, cand_ids = data_vec
        queries, query_words, query_marks, query_lengths, memories_vec, cand_ans_types = vectorize_data(queries,
                                                                                                        query_mentions,
                                                                                                        query_marks,
                                                                                                        memories, \
                                                                                                        max_query_size=
    ...
    def get_final_answer_score(self, kbqa_score, answer, similar_recipes, max_kbqa_score=1, min_kbqa_score=0):
        similarity_distance, self.similarity_var = self.get_recipe_similarity_distance(answer, similar_recipes)
        ...
        if self.similarity_var is None:
            similarity_score_ratio = self.config.get('similarity_score_ratio', 0.2)
        else:
            similarity_score_ratio = self.config.get('similarity_score_ratio', 0.2) * (1 - self.similarity_var)
    ...
    def get_recipe_similarity_distance(self, answer, similar_recipes):
        ...
        if similarity_distance is None:
            ret_sim = self.recipe_similairty.get_cosine_distance(answer, similar_recipes)
            similarity_distance = KBQA.max_similarity_distance if ret_sim is None else np.percentile(ret_sim, 0.5)

        ret_sim_var = None if ret_sim is None else ret_sim.var()

        return similarity_distance, ret_sim_var
    ...
    def get_ingredient_names(self, recipe_name):
        ...
        if dish_graph is not None:
            ingredients = {list(each.values())[0]['name'][0].lower() for each in
                           list(dish_graph.values())[0]['neighbors']['contains_ingredients']}
            return ingredients
    ...
    def answer(self, question, question_type, topic_entities, entities,
        ...
        if question_type == 'simple':
            if len(topic_entities) >= 1:
                answer_list, answer_id_list, rel_path_list, query_attn = self.simple_answer(question, topic_entities[0],
                                                                                            entities)
                err_code = 0
                err_msg = ''
            else:
            ...
            if len(topic_entities) >= 2:
                answer_list, answer_id_list, rel_path_list, query_attn = self.comparision_answer(question,
                                                                                                 topic_entities[0],
                                                                                                 topic_entities[1],
                                                                                                 entities)
                err_code = 0
            ...
            if len(topic_entities) >= 1:
                preferred_answer_type = None if self.config.get('no_filter_answer_type', False) else set(
                    ['dish_recipe'])
